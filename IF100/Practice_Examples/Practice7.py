...
# these were some datas about Turkey's 2002 election

city=int(input("Please enter an index between 0 and 80 (both are inclusive): "))
if 0<=city<=80:
  print("Results for", electoralDistricts[city] ,"as follows:")
  mylist=[0]*len(partyNames)
  vote=voteNumbersOfParties[city]
  for x in range(districtDeputyNumbers[city]):
    maximum=max(vote)
    idx=vote.index(maximum)
    mylist[idx]+=1
    vote[idx]=(maximum*mylist[idx])/(mylist[idx]+1)
  for y in range(len(partyNames)):
    print(partyNames[y]+":", mylist[y])
else:
...
down=int(input("Please enter a lower bound for the threshold (barrage): "))
up=int(input("Please enter an upper bound for the threshold (barrage): "))
lis1=[]
lis2=[]
lis3=[]
lis4=[]
...
for x in range(down,up+1):
  deputy=[0]*len(partyNames)
  for y in range(0,81):
    votes=voteNumbersOfParties[y][:]
    for idx in range(len(countrywidePercentages)):
      if x>countrywidePercentages[idx]:
        votes[idx]=0
    strict=[0]*len(partyNames)
    for z in range(districtDeputyNumbers[y]):
      maximum=max(votes)
      idx1=votes.index(maximum)
      deputy[idx1]+=1
      strict[idx1]+=1
      votes[idx1]=(maximum*strict[idx1])/(strict[idx1]+1)
  lis1.append(deputy[0])
  lis2.append(deputy[1])
  lis3.append(deputy[2])
  lis4.append(deputy[3])
  lis5.append(deputy[4])
  print(deputy)
plt.plot(range(down,up+1),lis1,label="Adalet ve Kalkınma Partisi")
plt.plot(range(down,up+1),lis2,label="Cumhuriyet Halk Partisi")
plt.plot(range(down,up+1),lis3,label="Doğru Yol Partisi")
plt.plot(range(down,up+1),lis4,label="Milliyetçi Hareket Partisi")
plt.plot(range(down,up+1),lis5,label="Genç Parti")
plt.xlabel("Barage (threshold)")  

...
if __name__ == "__main__":
   # get an input from command line
    ...
    with open('./output/cnf1_'+file_name+'.txt', 'w') as f:
    ...
    os.system("./kissat "+ './output/cnf1_'+file_name+'.txt' + " > " + './output/out1_'+file_name+'.txt')

    #read the output file and print the output
    sat = False
    with open('./output/out1_'+file_name+'.txt', 'r') as f:
        lines = f.readlines()
        for line in lines:
            if line[0] == 's':

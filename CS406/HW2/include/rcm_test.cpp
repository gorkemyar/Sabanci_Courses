...
//
{
  ...
  int* crs_ptrs_new2 = new int[node_num+1];
  int* crs_colids_new2 = new int[adj_num];

  crs_ptrs_new2[0] = 0;
  for (int i = 0; i < node_num; i++){  
    crs_ptrs_new2[i+1] = crs_ptrs_new2[i] + adj_row[perm[i]] - adj_row[perm[i] - 1];
    int start = crs_ptrs_new2[i];
    cout<<crs_ptrs_new2[i+1]<<" "<<adj_row[perm[i]]<<" "<<adj_row[perm[i] - 1]<<endl;
    for (int j = adj_row[perm[i]-1]; j < adj_row[perm[i]]; j++ ){
      crs_colids_new2[start] = perm_inv[adj[j-1]-1]-1;
      start++;
    }
  }
  cout<<"Error"<<endl;
  for (int i = 0; i < node_num; i++){  
    for (int j = crs_ptrs_new2[i]; j < crs_ptrs_new2[i+1]; j++ ){
      cout<<crs_colids_new2[j]<<" ";
    }cout<<endl;
  }

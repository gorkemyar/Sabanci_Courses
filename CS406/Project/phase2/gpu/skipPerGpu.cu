...
__global__ void calculation_skipper(double* matrix, int* cptrs, int* rowids, double* cvals, double*row_sum, double* d_result, int N, int NNZ, long loopVariant, long chunk){
    ...
    double* my_x = (double*) &shared_x[idx * N];
    if (threadIdx.x < N){
    ...
    long b = chunk*id;
    if (b > loopVariant) return;
    if (id == 0) b+=1;
    long limit = min((long)(id + 1l)*chunk, loopVariant);
        ...
        while (grdiff > 0){
            if (grdiff & 1){
                ...
                for(int ptr = l_cptrs[j]; ptr < l_cptrs[j+1]; ptr++){
                    my_x[l_rowids[ptr]] += (s * l_cvals[ptr]);
                }
            }
            grdiff = grdiff >> 1;
            j++;
    ...
    //d_result[blockIdx.x] = my_result;
    atomicAdd(&d_result[blockIdx.x], my_result);
}

__host__ double SkipPerGpu(double* matrix, int* rptrs, int* colids, double* rvals, int* cptrs, int* rowids, double* cvals, int nonzeros, int N, int NNZ)
{      
    cudaSetDevice(0);
    /// Device Pointers
    int block_size = 256;
    ...
    // Put Row Sum to Device Memory
    double *row_sum, *d_row_sum;
    double result = 1;
    ...
    long chunk = (loopVariant + (thread_count * block_size) - 1) / (thread_count * block_size);
    long shared_mem_size = thread_count * sizeof(double) * N;
    calculation_skipper<<<block_size, thread_count, shared_mem_size>>>(d_m, d_cptrs,  d_rowids,  d_cvals, d_row_sum, d_result, N, NNZ, loopVariant, chunk);

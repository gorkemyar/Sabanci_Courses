...
struct AABB{
    ...
    // minimum aabb that cover two aabbs
    // when ı was doing hw ı did not notice operator+ which is doing same operation with this function
    AABB unions(const AABB & rhs){
        int minx, miny, maxx, maxy;
        minx= (minX >=rhs.minX) ? rhs.minX: minX;
        miny= (minY >=rhs.minY) ? rhs.minY: minY;
        maxx= (maxX >=rhs.maxX) ? maxX: rhs.maxX;
        maxy= (maxY >=rhs.maxY) ? maxY: rhs.maxY;
